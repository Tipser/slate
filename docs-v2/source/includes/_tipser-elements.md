...
[Learn more](#product-tag).

## `Collection`

Renders a collection of product tiles based on [collectionId](#getting-tipser-ids) prop.

If the collection has many elements and you want to display it in one row as a collection, you need to add `carousel` prop. You can also use 'imgSize' prop to control the size of displayed product tiles. [Learn more](#collection).
...
<aside class="notice">
<code>ProductThumbanils</code> component must be placed in a container with fixed width and height.
</aside>
